...
template <class T>
T& Matrix<T>::operator()(std::size_t rows, std::size_t column) const {
...
template <class T>
void Matrix<T>::initMatrix(std::string path, bool t) {
    std::ifstream file(path);
    if (t) {
        if (file) {
            for (int i = 0; i < rows_; i++) {
...
template <class T>
void Matrix<T>::initMatrix(std::string path, Matrix<T>& wall_down) {
    std::ifstream file(path);
    if (file) {
        std::string line;
        for (std::size_t i = 0; i < rows_; i++) {
